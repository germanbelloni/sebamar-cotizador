...
run(
  "Puertas Herrero",
  `node "${path.join(scriptsDir, "impPuertasHerrero.js")}`,
);
...
run(
  "Ventanas Herrero",
  `node "${path.join(scriptsDir, "impVentanasHerrero.js")}`,
);
run(
  "Ventanas Modena",
  `node "${path.join(scriptsDir, "impVentanasModena.js")}`,
);
// ⚠️ superficies se maneja manualmente
// NO importar desde Excel
